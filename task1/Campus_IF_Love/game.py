...
class Game:
    ...
    def scene_xiaobai(self):
        ...
        if choice == "1":
            print(
                "\n你的目光掠过她草稿纸上那些代码，几乎立刻就看到了症结所在。“这里，”你刻意压低声音，指出了那个被忽略的条件，缓缓开始细心讲解。\n"
            )
            print(
                "小白先是抬头看你，愣了一下，随后立马跟上了你的思路。你进入【小白线】！"
            )
            self.current_target = self.characters["小白"]
            self.story_loop()
            return True
        else:
            print("\n你收回了目光，随意点出了一些要点，便迈开脚步径直走开。")
            return False
    ...
    def scene_jiejie(self):
        ...
        if choice == "1":
            print(
                "\n你的目光在她的脸上停留片刻后，缓缓低下了眉眼，身体向后靠进椅背，缓缓开始讲解，偶尔用指尖虚点屏幕上的关键点"
            )
            print(
                "姐姐听得很认真，偶尔“嗯”一声，或提出一个很切入要点的问题。你进入【姐姐线】！"
            )
            ...
            print(
                "\n你的目光没有丝毫偏移，依旧牢牢锁在屏幕上，对她的问题没有给出任何回应。"
            )
            return False
    ...
    def story_loop(self):
        ...
        while True:
            ...
            # 输入2----
                ...
                print(
                    "请选择你要送她的礼物：鲜花 / 编程笔记 / 奶茶 / 奇怪的石头 / 精致的钢笔 / 可爱玩偶 / 夜宵外卖"
                )
                gift = input().strip()
            ...
            # 输入3----
                ...
                print(
                    f"当前{self.current_target.name}的好感度为：{self.current_target.affinity}"
                )
            ...
            if (
                self.current_target == self.characters["None"]
                or self.current_target.check_ending()
            ):
                break
